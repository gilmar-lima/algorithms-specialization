 ...
 public:
  ...
  void CalculateSimplex() {
    ...
    for (int i = 0; i < A[0].size(); i++) {  // every basic column has the values, get it form B array
      ...
      if (count0 == rows - 1) {
        cout << "variable" << i + 1 << ": " << B[index]
             << endl;  // every basic column has the values, get it form B array
      } else {
...
int main() {
  ...
  float C[] = {1,-2,0,0,0};     // should initialis the c arry here
  float B[] = {1, 2, 2};  // should initialis the b array here
  ...
  std::vector<std::vector<float> > vec2D(rowSizeA,
                                         std::vector<float>(colSizeA, 0));
